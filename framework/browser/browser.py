...
class Browser(metaclass=Singleton):
    ...
    def set_up_driver(self, browser_key=BrowserConfig.BROWSER, capabilities=None, is_incognito=False,
                      enable_performance_logging=False, test_name=None, grid_port=None) -> None:
    ...
    def select_browser(self, browser_key=BrowserConfig.BROWSER) -> None:
    ...
    def quit(self, browser_key=BrowserConfig.BROWSER) -> None:
    ...
    def maximize(self, browser_key=BrowserConfig.BROWSER) -> None:
    ...
    def execute_script(self, script, arguments=None) -> dict:
    ...
    def switch_to_window(self, window_handle=None) -> bool | None:
    ...
    def switch_new_window(self, logged_page_name="") -> None:
    ...
    def switch_to_frame_by_name(self, frame_name) -> None:
        Logger.info("Переключение на фрейм с именем %s" % frame_name)
        self.get_driver().switch_to_frame(frame_name)

    def switch_to_frame_by_locator(self, search_condition, locator) -> None:
    ...
    def wait_for_custom_event(self,
                              event,
                              expected_result,
                              *args, time_in_seconds=Waits.IMPLICITLY_WAIT_SEC, msg="") -> None:
    ...
    def wait_for_true(self, expression, time_in_seconds=Waits.IMPLICITLY_WAIT_SEC, msg="") -> None:
    ...
    def wait_for_true_with_action(self, expression, event, expression_args=None, action_args=None,
                                  time_in_seconds=Waits.IMPLICITLY_WAIT_SEC, msg="") -> None:
        waiter = WaitForTrueWithAction(self.get_driver(), expression, event, expression_args=expression_args,
                                       action_args=action_args)
        self.wait_for_check_by_condition(waiter=waiter, time_in_seconds=time_in_seconds, message=msg)

    def wait_for_check_by_condition(self, waiter, time_in_seconds=Waits.IMPLICITLY_WAIT_SEC, message="") -> None:
        try:
    ...
    def add_cookie(self, cookie_name, cookie_value) -> None:
